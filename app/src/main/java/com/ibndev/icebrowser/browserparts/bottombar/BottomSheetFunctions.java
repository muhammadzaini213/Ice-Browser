...
public class BottomSheetFunctions {
    Activity activity;
    TabManager tabManager;
    ShowBookmarks showBookmarks;

    public BottomSheetFunctions(Activity activity, TabManager tabManager, SQLiteDatabase placesDB) {
        this.activity = activity;
        this.tabManager = tabManager;
        showBookmarks = new ShowBookmarks(activity, tabManager, placesDB);
    ...
    public void cookie(CookieManager cookieManager, boolean isCookieActive) {
        if (tabManager.getCurrentWebView() != null) {
            if (isCookieActive) {
                cookieManager.setAcceptThirdPartyCookies(tabManager.getCurrentWebView(), false);
            } else {
                cookieManager.setAcceptThirdPartyCookies(tabManager.getCurrentWebView(), true);
            }
        }
    }
